    ...
    <div className="flex-1 min-w-0">
      <div className="flex justify-between items-center">
        <h4 className="text-sm font-medium truncate text-gray-200">
          {session.title || 'Nova Análise CFA'}
        </h4>
      </div>
...
const FinancialAI: React.FC<FinancialAIProps> = ({ onOpenMenu }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = StorageService.getChatSessions(user.uid, 'financial', (loaded) => {
  ...
  const createNewSession = async () => {
    ...
    await StorageService.saveChatSession(newSession, 'financial', user.uid);
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await GeminiService.generateFinancialAnalysisStream(
        textToSend,
      ...
      await StorageService.saveChatSession(finalSession, 'financial', user.uid);
      
      if (finalSession.messages.length <= 2) {
        const title = await GeminiService.generateTitle(textToSend || "Análise Financeira");
        const titledSession = { ...finalSession, title };
        await StorageService.saveChatSession(titledSession, 'financial', user.uid);
      }
    } catch (e) {
      console.error(e);
    } finally {
      ...
      `}>
        <div className="p-3">
          <div className="flex items-center justify-between mb-2 lg:hidden px-2 pt-2">
             ...
             <span className="font-bold">CFA AI</span>
          ...
          >
            ...
            <span className="text-gray-500 group-hover:text-white text-xs">⌘K</span>
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-1 custom-scrollbar">
          <h3 className="text-[11px] font-bold text-gray-500 px-4 py-2 mt-2 uppercase tracking-wider">Histórico</h3>
          {sessions.length === 0 ? (
            <div className="p-4 text-center text-xs text-gray-600">Nenhuma análise iniciada</div>
          ) : (
            sessions.map(s => (
              <ChatListItem 
      ...
      `}>
        {currentSession ? (
          <>
            <header className="h-14 px-4 flex items-center justify-between shrink-0 bg-black z-20">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold border border-white/10">
                    CFA
                 </div>
              </div>
            </header>
                       ...
                       <div className={`max-w-[85%] lg:max-w-[75%] flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                          <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 border border-white/10 overflow-hidden">
                             {m.role === 'user' 
                               ...
                               : <div className="w-full h-full bg-blue-600 flex items-center justify-center"><span className="text-white font-bold text-xs">AI</span></div>
                             }
                          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center opacity-40">
            <div className="w-24 h-24 bg-blue-600/20 text-blue-500 rounded-[20px] flex items-center justify-center text-4xl mb-6 shadow-2xl border border-blue-500/20">
              📈
            </div>
            <h3 className="text-xl font-bold text-white mb-2">CFA Analyst Mode</h3>
            <p className="text-sm font-medium text-gray-400 max-w-xs leading-relaxed">Carregue gráficos ou peça análises de tendências de mercado.</p>
          </div>
        )}
      </div>
    </div>
